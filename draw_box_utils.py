...
STANDARD_COLORS = [
    'Aqua', 'Chartreuse', 'Pink', 'Red', 'Cyan',
]
# ------------------------------visualize degree estimation
# degree_extimation_STANDARD_COLORS = [
#     'Red', 'Gold', 'Cyan', 'Pink', 'Coral', 'Chartreuse'
# ]
...
def draw_text(draw,
    ...
    # instead of above.
    display_str = f"{category_index[str(cls)]}: {int(100 * score)}%" # example: car:0.98%
    # display_str = f"{round(score,3)}"
...
def draw_objs(image: Image,
    ...
    colors = [ImageColor.getrgb(STANDARD_COLORS[(cls-1) % len(STANDARD_COLORS)]) for cls in classes] # color is a list, color[0]=(0,255,255), index of color is 1~5
    # ------------------------------visualize degree estimation
    # colors = [ImageColor.getrgb(degree_extimation_STANDARD_COLORS[i]) for i in range(len(degree_extimation_STANDARD_COLORS))] # color is a list, color[0]=(0,255,255), index of color is 1~5
    ...
    if draw_boxes_on_image:
        ...
        for box, cls, score, color in zip(boxes, classes, scores, colors):
            ...
            draw.line([(left, top), (left, bottom), (right, bottom),
                       (right, top), (left, top)], width=line_thickness, fill=color) # line_thickness=3
            # 绘制类别和概率信息
