...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head,
                 ...
                 pre_nms_top_n, post_nms_top_n, nms_thresh, score_thresh=0.0, fuzzy=None):
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets, anchors=None, trans_maps=None):
        # type: (Tensor, Tensor, List[Tensor], List[Tensor], List[Tensor], Tensor) -> Tuple[Tensor, Tensor]
        """
        ...
        sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(labels) # sampled_pos_inds_0 [225603,1], element=1, the index refer to the positive
        # 将一个batch中的所有正负样本List(Tensor)分别拼接在一起，并获取非零位置的索引
        # sampled_pos_inds = torch.nonzero(torch.cat(sampled_pos_inds, dim=0)).squeeze(1)
        sampled_pos_inds = torch.where(torch.cat(sampled_pos_inds, dim=0))[0] # extract the positive index
        ...
        objectness_loss = F.binary_cross_entropy_with_logits(
            objectness[sampled_inds], labels[sampled_inds], weight=None
        )
    ...
    def forward(self,
        ...
        objectness, pred_bbox_deltas = self.head(features)  # feature:[2,512,42,84]

        # 生成一个batch图像的所有anchors信息,list(tensor)元素个数等于batch_size
        anchors = self.anchor_generator(images, features) # img_0:225603,4
        ...
        # 筛除小boxes框，nms处理，根据预测概率获取前post_nms_top_n个目标, 每个proposal不涉及类别概念
        boxes, scores = self.filter_proposals(proposals, objectness, images.image_sizes, num_anchors_per_level)  # boxes_0:[2000,4]
        ...
        if self.training:
            ...
            loss_objectness, loss_rpn_box_reg = self.compute_loss(
                objectness, pred_bbox_deltas, labels, regression_targets, anchors, trans_maps
            )
            losses = {
