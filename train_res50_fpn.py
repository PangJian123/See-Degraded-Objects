...
def create_model(num_classes, load_pretrain_weights=True, fuzzy=None):
    ...
    model = FasterRCNN(backbone=backbone, num_classes=num_classes, detectron2_roi_res5=detectron2_roi, dark=None, fuzzy=fuzzy)
...
def main(args):
    ...
    for epoch in range(args.start_epoch, num_epochs, 1):
        ...
        detloss, lr, = utils.train_one_epoch(model, optimizer, train_data_loader,
        ...
        with open(results_file, "a") as f:
            ...
            result_info = [f"{i:.4f}" for i in coco_info + [detloss.item()]] + [f"{lr:.6f}"]
        ...
        if current_map < coco_info[1]:
            ...
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, weights_dir + "mobile-model-best.pth")
        ...
        if (epoch+1) % 10 == 0:
            save_files = {
                ...
                'lr_scheduler': lr_scheduler.state_dict(),
                'epoch': epoch}
            torch.save(save_files, weights_dir + "mobile-model-{}.pth".format(epoch))
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(VOCdevkit)
    parser.add_argument('--data-path', default="/media/pj/UPC1/Datasets/RTTS_VOC/", help='dataset')

    parser.add_argument('--data-path-RTTS', default="/media/pj/UPC1/Datasets/RTTS_DCP/", help='dataset')
    ...
    parser.add_argument("--prior", default='trans', type=str) # trans / ams / weight_fusion / inversedepth
